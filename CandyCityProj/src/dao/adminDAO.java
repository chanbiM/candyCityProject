...
public class adminDAO {
	...
	public ArrayList<NoticeVO> getNoticeIndex(String type) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "select ROWNUM, a.* from (SELECT * FROM manager_notice where type=? AND end_o IS null ORDER BY notice_code desc) a where ROWNUM <=9";
	...
	public int updateEnd(String noticeCode) {
		int n = 0;
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "update manager_notice set end_o='O' where notice_code=?";
